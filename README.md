...
- **outpatient_claims:** Each row is a unique claim from a patient in one of these areas: ER visits, same-day/ambulatory surgeries, hospital clinic visits, diagnostic testing, and rehab therapies. The table is under pt. B of Medicare. This table covers: physician and provider information, procedure codes, diagnosis codes, coinsurance and deductible payment amounts, and HCPCS cost codes.
...
- **prescript_drug_events:** Each row is a unique healthcare claim about a prescription drug order from a specific patient that was handled by the pharmacies. This table solely covers pt. D of Medicare. This table covers: order number, order date, patient id, the type of drug dispensed, the quantity, the day supply, the patient payment amount, and the gross drug cost.
