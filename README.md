# Medicare DESynPUF Analysis (MLR, PMPM Cost, Count PMPM), 2008â€“2010
...
The Pennsylvania Department of Aging, specifically the Secretaryâ€™s Office and the Office of Policy & Planning, engaged a third-party healthcare analytics consultant to develop a dashboard and report. The project assesses the performance, accessibility, and equity of Medicare Parts B and D and Total (all service lines) at the national level, with the intent to later compare these findings to Pennsylvaniaâ€™s state-level performance. The analysis uses the [CMS DE-SynPUF Medicare 2008â€“2010 dataset](https://www.cms.gov/data-research/statistics-trends-and-reports/medicare-claims-synthetic-public-use-files/cms-2008-2010-data-entrepreneurs-synthetic-public-use-file-de-synpuf) to measure year-over-year (YoY) trends in **PMPM Cost**, **Count PMPM**, and **Medical Loss Ratio (**MLR**)** across Medicare service lines (svc) and 5 race groups. Given the parallel KPI declines by 2010, the analysis considers whether these trends reflect access barriers (cost sharing, prior authorization, network limits) or beneficial shifts (prevention, price reductions). The findings aim to guide decisions on outreach funding, transportation support, and benefits counseling.
...
- The svc RX was the only svc with declines in **MLR** every year (**-23.3%** YoY 08->09 and **-43.2%** YoY 09->10). RXâ€™s **MLR** fell from **552.2%** (2008) to **240.7%** in (2010). 
...
- Just like the svc Total, OPâ€™s and PROFâ€™s **MLR** values peaked in 2009 at **95.9%** and **173.6%**, then fell in 2010 to **47.2%** and **97.2%**. In 2010, **MLR** declined across all service lines (YoY): Total **-45.5%**, OP **-50.8%**, PROF **-44.0%**, RX **-43.2%**.

- Allowed Cost is Medicareâ€™s total annual spending on beneficiariesâ€™ care and the numerator of **MLR**. Based on the [Allowed Cost Distribution Graph](./assets/allowed_cost_distribution_graph.png) for OP, PROF, and RX, the relative distribution of Allowed Cost was stable, with a mean absolute YoY %Î” of **4.2%**.

***Disclaimer:*** High **Medical Loss Ratio (**MLR**)** values in the **200â€“300%** range in this report are not errors; they reflect that, unlike private insurers, Medicare is not funded solely by premiums. MLRs below **85%** arenâ€™t errors, Part B premiums can fully cover some service lines (e.g. OP), but this service-line view excludes the combined OP + PROF cost.

[![Insights Deep Dive](./assets/data_reads1.png)](./assets/data_reads1.png)

### **MLR** by Race Group and SVC:

- Total **MLR** by race group: All Beneficiaries (AB), Black, Caucasian, and Hispanic are tightly clustered, peaking in 2009 at **301.0â€“305.2%**. The Other group peaks lower at **272.7%** (vs. AB **301%**) in 2009. In 2010 all groups declined by a similar margin (YoY **-44.1%** to **-45.6%**).

- OP **MLR** by race group: AB, Black, Caucasian, and Hispanic are tightly clustered, peaking in 2009 at **90.8â€“97.8%**; Other peaks lower at **81.5%** (vs. AB **95.9%**). In 2010, all groups declined by a similar amount (YoY **-49.0%** to **-50.9%**). 

- PROF **MLR** by race group: The five groups are fairly evenly spaced, with inter-group ranges of **~15â€“32** points year over year. **MLR** peaked in 2009 at **144.5â€“176.9%** (Other **144.5%** vs. AB **173.6%**). In 2010 all groups declined similarly (YoY **-42.9%** to **-44.6%**).

- RX **MLR** by race group: The Hispanic groupâ€™s **MLR** is consistently higher than the other four, with the widest gap in 2009 (**526%** vs. Black **451%**, **+75 pts**). RX MLRs peak in 2008 (**545.6â€“600.2%**). Again, in 2010 all groups declined similarly (YoY **-40.8%** to **-43.4%**).

**Tip:** To see these referenced values, hover over the graphs while in the dashboard. The tooltip on these graphs shows both the actual values and the YoY %Î”, while other graphs show only the actual values.
...
- **Count PMPM** uses the same denominator as **PMPM Cost**, member-months, but a different numerator (claim counts, not dollars). Even so, it mirrors the **MLR**/**PMPM Cost** pattern: PROF, OP, and Total peak in 2009 at **1.5**, **0.3**, and **3.4 claims PMPM**, then drop sharply in 2010, while RX peaks earlier at **2.5** in 2008 and declines thereafter. In 2010, **Count PMPM** falls YoY across all service lines: Total **-35.8%**, OP **-44.0%**, PROF **-36.1%**, RX **-39.7%**.

- Claim Count is the annual total number of claims and serves as the numerator of **Count PMPM**. Based on the [Claim Count Distribution Graph](./assets/claim_count_distribution_graph.png) Across OP, PROF, and RX, its service-line share was stable, with a mean absolute YoY %Î”  of **3.5%**.
...
- [Total **Count PMPM**](./assets/total_claim_pmpm_rg_graph.png) by race group mirrors the other KPIs: AB, Black, Caucasian, and Other are tightly clustered, peaking in 2009 at **3.3â€“3.4 claims PMPM**, while Hispanic peaks higher at **3.8** (vs. AB **3.4**). In 2010, all groups decline by a similar amount (YoY **-34.8%** to **-35.9%**).
...
Based on the insights and findings above, we recommend the Pennsylvania Department of Aging, specifically the Secretaryâ€™s Office and the Office of Policy & Planning, consider the following:
...
* **Premium Revenue Estimation Breakdown:** Premium revenue is the denominator (component measure) of **MLR**. For each beneficiary, it equals the months of Part B or Part D enrollment during the year multiplied by that yearâ€™s national average monthly premium for that part, then summed across beneficiaries. This simplification of premium revenue was necessary because the dataset lacks beneficiary-specific monthly premium amounts by Medicare part. By contrast, Allowed Cost, the **MLR** numerator, required no simplification and reflects the claims spend as reported.
